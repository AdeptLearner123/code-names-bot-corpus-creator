    ...
    with tqdm(total=len(target_pages)) as pbar:
        for i in range(0, len(target_pages), CHUNK_SIZE):
            ...
            with ThreadPoolExecutor(max_workers=len(target_pages)) as ex:
                futures = [
                    ex.submit(save_words_from_page, page_id, results)
            ...
            for page_id in results:
                if len(results[page_id]) > 0:
                    with open(
                        os.path.join(OXFORD_WORD_LIST_DIR, f"{page_id}.txt"), "w+"
                    ) as file:
...
def main():
    ...
    scraped_pages = set(
        map(lambda file_name: int(file_name.split(".")[0]), scraped_pages)
    )
    target_pages = range(1, TOTAL_PAGES + 1)
    target_pages = list(
        filter(lambda page_id: page_id not in scraped_pages, target_pages)
    )
