...
from config import PREMIUM_OXFORD_WORD_LIST_DIR, PREMIUM_OXFORD_WORD_LIST_TARGETS_DIR

GET_URL = lambda item: f"https://premium-oxforddictionaries-com.offcampus.lib.washington.edu/us/browse/american_english/{item[0]}/{item[1]}/"
...
def format_word(word):
    parts = word.split("\u2013")  # Preserve en dash
    parts = list(map(lambda part: unidecode(part).lower().replace(" ", "-").replace(",", ""), parts))
    return "\u2013".join(parts)
...
def get_targets():
    ...
    for file_name in os.listdir(PREMIUM_OXFORD_WORD_LIST_TARGETS_DIR):
        ...
        with open(os.path.join(PREMIUM_OXFORD_WORD_LIST_TARGETS_DIR, file_name), "r") as file:
            targets += list(map(lambda first_word: (letter, format_word(first_word)), file.read().splitlines()))
    return targets


def main():
    for file_name in os.listdir(PREMIUM_OXFORD_WORD_LIST_DIR):
        tuple = make_tuple(file_name.split(".txt")[0])
        print(tuple, file_name)
        os.rename(file_name, tuple[0] + "|" + tuple[1] + ".txt")

    return

    targets = get_targets()

    scrape(GET_URL, targets, extract, PREMIUM_OXFORD_WORD_LIST_DIR, headers={
