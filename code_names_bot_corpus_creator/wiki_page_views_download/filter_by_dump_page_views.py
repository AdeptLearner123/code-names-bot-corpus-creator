...
def main():
    ...
    with open(WIKI_PAGES) as file:
        ...
        page_id_titles = list(map(lambda page_id_title: page_id_title.split("\t"), page_id_titles))
    
    filtered_page_id_titles = list(filter(lambda page_id_title: page_views[page_id_title[1]] > PAGE_VIEW_THRESHOLD, page_id_titles))
    filtered_page_id_titles = list(map(lambda page_id_title: "\t".join(page_id_title), filtered_page_id_titles))
