...
def has_short_title_or_acronym(title, redirects):
    return len(title.replace("_", " ").split(" ")) <= 2 or get_acronym(title) in redirects
...
def main():
    ...
    titles = filter(lambda title: has_short_title_or_acronym(title, title_to_redirects[title]), titles)
